...
bm <- glm(IS_BUY ~ WEEKDAY, 
          data = select(treino, WEEKDAY, IS_BUY), 
          family = "binomial")
...
```{r}
predictions <- predict(bm, type = "response") > .05
verdadeiras_compras <- treino$IS_BUY == 1
 
table(predictions, verdadeiras_compras)
```

Como o problema é bastante desbalanceado (95% para não compras e 5% para compras) é importante que o o erro falso positivo seja o menor possível. No modelo acima ele errou (8950*100/8950+21423) 3.3% nas previsão de compras. Ele devia ter classificado como compra e classificou como não compra. 

Em busca de melhorar esse número, criamos outro modelo. Dessa vez considerando a categoria e o dia da semana. 


```{r}
bm2 <- glm(IS_BUY ~ WEEKDAY + CATEGORY, 
          data = select(treino, WEEKDAY, IS_BUY, CATEGORY), 
          family = "binomial")
