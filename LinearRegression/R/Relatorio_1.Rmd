...
```{r, message=FALSE, warning=FALSE}
prostate <- read.delim("~/Projetos/DataAnalysis/LinearRegression/dados/prostate.data")
...
Iremos agora adicionar mais duas variáveis no nosso modelo e verificar se o nosso modelo fica melhor ou pior. Escolhemos as variáveis lcp e svi utilizando o mesmo critério anterior.
...
Com esse novo modelo temos um RMSE mais baixo do que o anterior. Isso pode ter resultado de uma variável pouco importante para o  modelo.

```{r, message=FALSE, warning=FALSE}
summary(mod_2)
```

Verificamos que a variável lcp tem pouca significancia para o modelo. Por isso, criamos um novo modelo sem essa variável. 
...
Verifiamos que novamente o modelo criado não é melhor do que o modelo base. 